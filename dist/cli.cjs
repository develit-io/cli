...
async function replaceContent(filepath, replacer) {
  const content = await fs__namespace.readFile(filepath, "utf8");
  await fs__namespace.writeFile(filepath, replacer(content));
}
function capitalize(str) {
  return str.charAt(0).toUpperCase() + str.slice(1);
}
const replaceTemplateContent = async (rootDir, projectName) => {
  await Promise.all([
    replaceContent(path__namespace.resolve(rootDir, "package.json"), (content) => content.replace("@services/template", projectName)),
    replaceContent(path__namespace.resolve(rootDir, "@types/index.ts"), (content) => content.replaceAll("Template", capitalize(projectName))),
    replaceContent(path__namespace.resolve(rootDir, "src/index.ts"), (content) => content.replace("class TemplateService", `class ${capitalize(projectName)}Service`)),
    replaceContent(path__namespace.resolve(rootDir, "wrangler.toml"), (content) => content.replace("template-service", `${projectName}-service`))
  ]);
};
...
const createCommand = defineCommand({
  ...
  async run(ctx) {
    ...
    const __targetDir = pathe.resolve(process.cwd(), projectName);
    const copyTemplateSpinner = p__namespace.spinner();
    try {
      console.log(projectName);
      copyTemplateSpinner.start("Creating new Service Worker...");
      await giget.downloadTemplate(`${DEFAULT_REGISTRY}#${template}`, {
        dir: __targetDir
      });
      ...
      if (installDeps) {
        try {
          ...
          p__namespace.log.error(`${error}`);
          p__namespace.outro("Operation failed.");
        }
      }
      p__namespace.outro(`Navigate to ${projectName} and start building! \u{1F680}`);
    } catch (error) {
